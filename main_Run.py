    ...
    def fill_attendance():
        ...
        if subb == "":
            ...
            def create_csv():
                ...
                csv_name = (
                    "C:/Users/Pragya singh/PycharmProjects/Attendace_management_system/Attendance/Manually Attendance/"
                    + DB_table_name
                    + ".csv"
                )
            ...
            try:
                while True:
                    ...
                    for x, y, w, h in faces:
                        ...
                        sampleNum += 1
                        cv2.imwrite(
                            "TrainingImage/" + Name + "." + Enrollment + "." + str(sampleNum) + ".jpg",
                            gray,
                        )
    ...
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == "":
                ...
                try:
                    recognizer.train("TrainingImageLabel\Trainner.yml")
                except:
                ...
                while True:
                    ...
                    faces = tcdetector.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5)
                    for x, y, w, h in faces:
                ...
                try:
                    global cursor
                    connection = pymysql.connect(
                        host="localhost", user="root", password="Rachit@31", db="Face_reco_fill"
                    )
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
                ...
                root.configure(background="grey80")
                cs = (
                    "C:/Users/Pragya Singh/PycharmProjects/Attendace_management_system/"
                    + fileName
                )
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == "pragya":
            ...
            valid = "Incorrect ID or Password"
            Nt.configure(
                text=valid, bg="red", fg="white", width=38, font=("times", 19, "bold")
            )
            Nt.place(x=120, y=350)
    ...
    pw_entr = tk.Entry(
        win, width=20, show="*", bg="white", fg="black", font=("times", 23)
    )
...
def trainimg():
    ...
    try: 
        ...
        if len(faces) == 0 or len(Id) == 0:
            print("Error: No faces or IDs to train.")
        return
        ...
        Notification.configure(
            text=l, bg="SpringGreen3", width=50, font=("times", 18, "bold")
        )
    ...
    # try:
        ...
        print("Value error during training:", ve)
    except MemoryError as me:
        print("Memory error during training:", me)
    except Exception as e:
    ...
    try:
        ...
        Notification.configure(
            text=q, bg="SpringGreen3", width=50, font=("times", 18, "bold")
        )
        Notification.place(x=350, y=400)

    res = "Model Trained"  # +",".join(str(f) for f in Id)
    Notification.configure(
        text=res, bg="olive drab", width=50, font=("times", 18, "bold")
    )
    Notification.place(x=250, y=400)
...
def getImagesAndLabels(path):
    ...
    for imagePath in imagePaths:
        # Assuming faceCascade is already defined and initialized
        # Load the image and convert it to grayscale
        pilImage = Image.open(imagePath).convert("L")
        imageNp = np.array(pilImage, "uint8")

        # Initialize the faces variable
        faces = []
        Id = int(os.path.split(imagePath)[-1].split(".")[1])

        # Perform face detection
        faces = faceCascade.detector.detectMultiScale(imageNp)

        # Assign detected faces to the faces variable

        # Now you can safely use the faces variable
        if len(faces) > 0:
            for x, y, w, h in faces:
...
txt = tk.Entry(
    window, validate="key", width=20, bg="white", fg="black", font=("times", 25)
)
